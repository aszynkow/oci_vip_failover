#!/usr/bin/env python3
"""Standalone removal of secondary private IPs from an OCI VNIC."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--ip", dest="ips", action="append", help="Secondary IPv4 address to remove. Repeat or comma-separate.")
    parser.add_argument("--all-secondary", action="store_true", help="Remove every non-primary private IP from the VNIC.")
    parser.add_argument("--dry-run", action="store_true", help="Show actions without deleting private IPs.")
    parser.add_argument(
        "--show-assigned",
        "--show-assigned-ips",
        action="store_true",
...
def main() -> int:
    ...
    private_ips = list_private_ips(oci, client, vnic_id)
        ...
        print(json.dumps(
            {
                ...
                "assigned_private_ips": assigned_ip_summaries(private_ips),
            },
            indent=2,
            sort_keys=True,
        ))
    ...
    requested_ips = [] if args.all_secondary else secondary_ips(args.ips, config)
    private_ips_by_address = {private_ip.ip_address: private_ip for private_ip in private_ips}
    if args.all_secondary:
        targets = [private_ip for private_ip in private_ips if not getattr(private_ip, "is_primary", False)]
    else:
        targets = [private_ips_by_address[ip] for ip in requested_ips if ip in private_ips_by_address]

    result: dict[str, Any] = {
        "vnic_id": vnic_id,
        "requested_ips": requested_ips if not args.all_secondary else "ALL_SECONDARY",
        "would_delete": [],
        "deleted": [],
        "not_found": [],
        "skipped_primary": [],
        "dry_run": args.dry_run,
        "mode": "DRY_RUN_NO_CHANGES" if args.dry_run else "DELETE",
    }

    if not args.all_secondary:
        target_addresses = {private_ip.ip_address for private_ip in targets}
        result["not_found"] = [ip for ip in requested_ips if ip not in target_addresses]

    if args.dry_run:
        for private_ip in targets:
            summary = private_ip_summary(private_ip)
            if getattr(private_ip, "is_primary", False):
                result["skipped_primary"].append(summary)
                continue
            summary["would_delete"] = True
            result["would_delete"].append(summary)
    ...
    for private_ip in targets:
        ...
        if getattr(private_ip, "is_primary", False):
            result["skipped_primary"].append(summary)
            continue
        client.delete_private_ip(private_ip.id)
